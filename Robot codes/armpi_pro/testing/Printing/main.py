...
def move_arm(x, y, z, in_time):
    ...
    if target:
        ...
        # Move the arm
        bus_servo_control.set_servos(joints_pub, in_time, ((1, 200), (2, 500), (3, servo_data['servo3']),
                        (4, servo_data['servo4']),(5, servo_data['servo5']),(6, servo_data['servo6'])))
...
def print_function(speed):
    ...
    for i in range(len(xVal)):
        ...
        i = int(i)
        trueX = xVal[i]/1000 - .15
        trueY = yVal[i]/1000 + .1
        trueZ = zVal[i]/1000 + z_off
        try:
            print('------------------------ Move #' + str(i) + ' ---------------------\n')
            
            # Calculates time to move for the currrent xVal[i]
            current_time = speed_to_time(xVal[i], xVal[i+1], yVal[i], yVal[i+1], speed) 
            
            # Moves the arm to the coordiantes of the current xVal[i]
            move_arm(trueX, trueY, trueZ, current_time)                 
            
            # Logs the current coordinates the arm is at
            print(('Moving to\n x: ') + ('{:.5f}'.format(xVal[i]/1000)) +            # Displays x-coordinate
                             ('\n y: ') + ('{:.5f}'.format(yVal[i]/1000)) +            # Displays y-coordinate
                             ('\n z: ') + ('{:.5f}'.format(zVal[i]/1000 + z_off)))     # Displays z-coordinate
