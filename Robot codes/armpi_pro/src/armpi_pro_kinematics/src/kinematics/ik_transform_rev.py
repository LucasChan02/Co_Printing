...
class ArmIK:
    ...
    def inverse_kinematics(self, coordinate_data, alpha):
        a, b, c = coordinate_data
        x = a + self.x_clearance
        y = b
        z = c + self.z_offset
        ...
        end_eff_angle = np.arctan(self.end_effector_x_offset / self.end_effector_link_length)
        gamma = (np.pi + pitch_angle - end_eff_angle)  
        ...
        l_cd = self.forearm_link_length
        l_de = self.upper_arm_length

        angle_d = np.pi/2 - np.arcsin((l_cd**2 + l_de**2 - l_ce**2) / (2 * l_cd * l_de))
        theta4 = np.rad2deg(angle_d - np.pi)    # elbow angle

        angle_e = np.pi/2 - np.arcsin((l_de**2 + l_ce**2 - l_cd**2) / (2 * l_de * l_ce))
        theta5 = np.rad2deg( -(np.pi/2 -angle_e - beta))  # shoulder angle

        angle_c = np.pi - angle_d - angle_e
        # theta3 = np.rad2deg( -(np.pi - angle_c - (np.pi/2 - beta) - (pitch_angle + np.pi)) - end_eff_angle)  # wirst angle
        theta3 = np.rad2deg( -(np.pi - angle_c - (np.pi/2 - beta) - (pitch_angle + np.pi)))
        ...
        if is_theta3_valid and is_theta4_valid and is_theta5_valid and is_theta6_valid:
            ...
            # debug message
            if not is_theta3_valid: print(f"Theta3 {theta3} is out of range")
            if not is_theta4_valid: print(f"Theta4 {theta4} is out of range")
            # ... etc.
            return False
