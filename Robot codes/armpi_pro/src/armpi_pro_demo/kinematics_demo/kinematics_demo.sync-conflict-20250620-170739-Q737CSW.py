...
if __name__ == '__main__':
    ...
    # 设置初始位置
    target = ik.setPitchRanges((0.0, 0.12, 0.15), -125, -180,90) # 运动学求解
    if target: # 判断是否有解
        servo_data = target[1]
        # 驱动机械臂移动
        bus_servo_control.set_servos(joints_pub, 1500, ((1, 200), (2, 500), (3, servo_data['servo3']),
    ...
    time.sleep(1.5)
    # X轴移动到0.15米处
    target = ik.setPitchRanges((-0.15, 0.12, 0.15),-125, -180, 90)
    if target:
        servo_data = target[1]
        bus_servo_control.set_servos(joints_pub, 1500, ((1, 200), (2, 500), (3, servo_data['servo3']),
                        (4, servo_data['servo4']),(5, servo_data['servo5']),(6, servo_data['servo6'])))
    time.sleep(1.5)
    # 回到初始位置
    target = ik.setPitchRanges((0.0, 0.12, 0.15), -125, -180, 90)
    if target:
        servo_data = target[1]
        bus_servo_control.set_servos(joints_pub, 1500, ((1, 200), (2, 500), (3, servo_data['servo3']),
                        (4, servo_data['servo4']),(5, servo_data['servo5']),(6, servo_data['servo6'])))
    time.sleep(2)
    # Y轴向前移动到0.2米处
    target = ik.setPitchRanges((0.0, 0.20, 0.15), -125, -180, 90)
    if target:
        servo_data = target[1]
        bus_servo_control.set_servos(joints_pub, 1500, ((1, 200), (2, 500), (3, servo_data['servo3']),
                        (4, servo_data['servo4']),(5, servo_data['servo5']),(6, servo_data['servo6'])))
    ...
    time.sleep(2)
    # Z轴向上移动到0.24米处
    target = ik.setPitchRanges((0.0, 0.12, 0.24), -125, -180, 90)
    if target:
        servo_data = target[1]
        bus_servo_control.set_servos(joints_pub, 1500, ((1, 200), (2, 500), (3, servo_data['servo3']),
                        (4, servo_data['servo4']),(5, servo_data['servo5']),(6, servo_data['servo6'])))
