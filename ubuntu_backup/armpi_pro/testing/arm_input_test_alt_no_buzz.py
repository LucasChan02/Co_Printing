...
-------------------------------------------------------------
  ...
  - This version moves as you input numbers
...
def move_arm(x, y, z):
    ...
    if isinstance(x, float) and isinstance(y, float) and isinstance(z, float):
        ...
        # Determine if there is a soluiton
        if target:
#             success_buzz()
            servo_data = target[1]
            ...
            print('Unable to find an arm solution from inputed coordinates\n')
    else:
...
if __name__ == '__main__':
    # Initialize node
    rospy.init_node('arm_input_test_no_buzz', log_level=rospy.DEBUG)
