...
where $t$ represents the layer thickness (layer height) and $\theta$ is the stratification angle. The parameters $\epsilon_x$ and $\epsilon_y$ are defined as error coefficients which quantify the deviation of layer thickness along the X and Y directions respectively. They are modeled as independent random variables following normal distributions determined through experimental measurements. These coefficients integrate the influence of various error sources \cite{ahn2009representation}, such as geometric inaccuracies, process parameter variations, machine imperfections, and environmental factors into the surface profile model.
